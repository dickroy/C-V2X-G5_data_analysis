...
'   - Restore bucket Nsch total constraint validation without rescanning the full dataset
...
Private Function EvaluatePoolBucketExcludingRow(ByVal sfnVal As Long, ByVal excludeRowIdx As Long) As Boolean
    ...
    t = MicroTimer_TXSFNCR()
    rows = ExtractCurrentBucketRows(excludeRowIdx, -1, rowCount)
    EvaluatePoolBucketExcludingRow = ValidateBucketRowsLocal(rows, rowCount, excludeRowIdx, -1)
...
Private Function EvaluatePoolBucketWithAddedRow(ByVal sfnVal As Long, ByVal addedRowIdx As Long) As Boolean
    ...
    t = MicroTimer_TXSFNCR()
    rows = ExtractCurrentBucketRows(-1, addedRowIdx, rowCount)
    EvaluatePoolBucketWithAddedRow = ValidateBucketRowsLocal(rows, rowCount, -1, addedRowIdx)
...
Private Function ExtractCurrentBucketRows(ByVal excludeRowIdx As Long, ByVal includeRowIdx As Long, ByRef rowCount As Long) As Long()
    Dim rows() As Long
    Dim i As Long
    Dim n As Long
    Dim rowIdx As Long
    ...
    If mPoolCount <= 0 Then
        ReDim rows(1 To 1)
        rowCount = 0
        ExtractCurrentBucketRows = rows
        Exit Function
    End If

    ReDim rows(1 To mPoolCount)
    rowCount = 0

    For i = 1 To mPoolCount
        rowIdx = mPoolRows(i)
        If rowIdx <> excludeRowIdx Then
            If includeRowIdx < 1 Or rowIdx = includeRowIdx Or mCurrentSFN(rowIdx) = mCurrentSFN(includeRowIdx) Then
                rowCount = rowCount + 1
                rows(rowCount) = rowIdx
            End If
        End If
    Next i
    ...
    If rowCount = 0 Then
        ReDim rows(1 To 1)
    ElseIf rowCount < mPoolCount Then
...
Private Function ValidateBucketRowsLocal(ByRef rows() As Long, ByVal rowCount As Long, ByVal excludedRowIdx As Long, ByVal addedRowIdx As Long) As Boolean
    ...
    Dim nschLimit As Long

    If rowCount <= 0 Then
        ValidateBucketRowsLocal = True
        Exit Function
    End If

    Set dictTx = CreateObject("Scripting.Dictionary")
    Set dictStations = CreateObject("Scripting.Dictionary")
    nschLimit = GetNamedLong("maxTX_SFN_est_increment")

    For i = 1 To rowCount
        rowIdx = rows(i)
        includeRow = True

        If excludedRowIdx > 0 Then
            If rowIdx = excludedRowIdx Then includeRow = False
        End If

        If includeRow Then
            If addedRowIdx > 0 Then
                If rowIdx = addedRowIdx Or mCurrentSFN(rowIdx) = mCurrentSFN(addedRowIdx) Then
                    ' included
                End If
            End If
            ...
            sumNsch = sumNsch + mRowNsch(rowIdx)
            If nschLimit > 0 Then
                If sumNsch > nschLimit Then Exit Function
            End If
        End If
    Next i
