...
Public Function FindConstraintViolations(ByVal numViolations2Find As Long) As Long
    ...
    Dim groupHasWarning As Boolean
    ...
    If numViolations2Find = 0 Then
        ...
        wsLog.Cells(7, baseOutCol).Value = "Num Groups with Constraint Violations (Types 1-" & NumConstraints & "):"
        wsLog.Cells(7, baseOutCol + 1).Value = 0
        wsLog.Cells(8, baseOutCol).Value = "Warning Groups (Types TXTIME/RXTIME; total types=" & NumWarnings & "):"
    ...
    Do While i <= UBound(data, 1)
                    ...
                    If dictTXProc.Exists(CStr(txStationId)) Then
                        ...
                        If txSfnVal < (Round(txQTime + txtProc) - maxTXDecrement) Then
                            ...
                            If numViolations2Find = 0 Then
                                ...
                                warningRows.Add Array(startRow, currentSFN, "TXTIME", "TX_SFN_est < ROUND(TXQTIME + TXTproc) - maxTX_SFN_est_decrement.")
                                writeRow = writeRow + 1
                            Else
            ...
            If rxMinSet Then
                ...
                If txSfnVal > rxThreshold Then
                    ...
                    If numViolations2Find = 0 Then
                        ...
                        warningRows.Add Array(j, currentSFN, "RXTIME", "TX_SFN_est > min(RXTIME) - RXTproc + 3*sigma.")
                        writeRow = writeRow + 1
                    Else
                        FindConstraintViolations = numViolations
                        Exit Function
        ...
        If Not IsUniqueLongList(txIDs, txCount) Then
            ...
            If numViolations2Find = 0 Then
                ...
                violationRows.Add Array(startRow, currentSFN, 1, "TX_ID values are not unique within this TX_SFN_est group.")
                writeRow = writeRow + 1
            Else
                FindConstraintViolations = numViolations
                Exit Function
        ...
        If HasOverlap(txIDs, txCount, rxStationIDs, rxCount) Then
            ...
            If numViolations2Find = 0 Then
                ...
                violationRows.Add Array(startRow, currentSFN, 2, "Merged TX + RX station list is not unique within this TX_SFN_est group.")
                writeRow = writeRow + 1
            Else
                FindConstraintViolations = numViolations
                Exit Function
        ...
        For st = 1 To nRx
            If capacitySum(st) > maxSch Then
                ...
                If numViolations2Find = 0 Then
                    ...
                    violationRows.Add Array(startRow, currentSFN, 3, "St " & st & " sum " & capacitySum(st) & " > " & maxSch)
                    writeRow = writeRow + 1
                Else
                    FindConstraintViolations = numViolations
                    Exit Function
...
NextGroup:
    ...
    If numViolations2Find = 0 Then
        ...
        writeRow = WriteFCVIssueSection(wsLog, writeRow + 2, "VIOLATIONS", violationRows)
        WriteFCVIssueSection wsLog, writeRow + 2, "WARNINGS", warningRows
        wsLog.Cells(1, baseOutCol).Font.Bold = True
