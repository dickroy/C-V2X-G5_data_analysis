...
Private mRowRxPresent() As Boolean
Private mRowRxNsch() As Long
...
Private Sub PrepareRowDerivedData()
    ...
    ReDim mRowRxPresent(1 To mFilteredCount, 1 To mActiveRxCount)
    ReDim mRowRxNsch(1 To mFilteredCount, 1 To mActiveRxCount)
    ...
    For r = 1 To mFilteredCount
        ...
        mRowMinRxStation(r) = GetRowMinRxStation(r)
...
Private Function GetRowMinRxStation(ByVal rowIdx As Long) As Long
    Dim st As Long
    Dim colIdx As Long
    Dim rxVal As Double
            ...
            If Not IsEmpty(mData(rowIdx, colIdx)) Then
                If IsNumeric(mData(rowIdx, colIdx)) Then
                    ...
                    If Not hasValue Then
                        hasValue = True
                        bestVal = rxVal
                        GetRowMinRxStation = st
                    ElseIf rxVal < bestVal Then
                        bestVal = rxVal
                        GetRowMinRxStation = st
                    End If
                End If
...
Private Sub CacheRowRxPresenceAndNsch(ByVal rowIdx As Long)
    Dim st As Long
    Dim colIdx As Long
    ...
    For st = 1 To mActiveRxCount
        If st <= UBound(mRxDataColIdx) Then
            colIdx = mRxDataColIdx(st)
            If colIdx > 0 Then
                If Not IsEmpty(mData(rowIdx, colIdx)) Then
                    If LenB(Trim$(CStr(mData(rowIdx, colIdx)))) > 0 Then
                        mRowRxPresent(rowIdx, st) = True
                        mRowRxNsch(rowIdx, st) = mRowNsch(rowIdx)
                    Else
                        mRowRxPresent(rowIdx, st) = False
                        mRowRxNsch(rowIdx, st) = 0
                    End If
                Else
                    mRowRxPresent(rowIdx, st) = False
                    mRowRxNsch(rowIdx, st) = 0
...
Private Function HasRowRxForStation(ByVal rowIdx As Long, ByVal stationOrdinal As Long) As Boolean
    ...
    HasRowRxForStation = mRowRxPresent(rowIdx, stationOrdinal)
End Function
...
Private Sub CacheRowRxPresenceAndNsch(ByVal rowIdx As Long)
    ...
    For st = 1 To mActiveRxCount
        If Not IsEmpty(mData(rowIdx, mRxDataColIdx(st))) Then
            If LenB(Trim$(CStr(mData(rowIdx, mRxDataColIdx(st))))) > 0 Then
                mRowRxPresent(rowIdx, st) = True
                mRowRxNsch(rowIdx, st) = mRowNsch(rowIdx)
            Else
                mRowRxPresent(rowIdx, st) = False
